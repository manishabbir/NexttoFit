...
export default function HomePage() {
  ...
  // React Query with 60s staleTime cache
  const { data } = useQuery({
    queryKey: ["homepage"],
    queryFn: () => fetch("/api/homepage").then((res) => res.json()),
    staleTime: 60 * 1000,
  });
  ...
  useEffect(() => {
    ...
    if (data.siteSettings) {
      ...
      if (s.features_bar) {
        const parsed = JSON.parse(s.features_bar);
      ...
      if (s.promo_banner) {
        try {
          const parsed = JSON.parse(s.promo_banner);
          if (parsed && typeof parsed === "object") setPromoBanner(parsed);
        } catch {}
      }
      if (s.social_section) {
        try {
          const parsed = JSON.parse(s.social_section);
    ...
    if (Array.isArray(data.bestsellerProducts) && data.bestsellerProducts.length > 0) {
                ...
                <div key={product.id} className="group relative">
                  <Link href={`/product/${product.slug}`}>
                    <div className="relative aspect-[3/4] overflow-hidden rounded-2xl bg-muted">
                      <OptimizedImage src={primaryImage?.url || product.images?.[0]?.url} alt={product.name} className="h-full w-full transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" />
                  ...
                  <div className="mt-4 space-y-1"><Link href={`/product/${product.slug}`} className="block text-sm font-medium transition-colors hover:text-gold-500">{product.name}</Link><div className="flex items-center gap-2"><span className="text-sm font-semibold">Rs.{Number(product.price).toLocaleString()}</span>{product.comparePrice && <span className="text-xs text-muted-foreground line-through">Rs.{Number(product.comparePrice).toLocaleString()}</span>}</div></div>
                </div>
              );
            })}
          </div>
      ...
      {promoBanner.enabled && (
        <section className="relative py-20 md:py-32 overflow-hidden">
          <div className="absolute inset-0">
            <OptimizedImage src={promoBanner.imageUrl} alt="Promo banner" className="h-full w-full" sizes="100vw" />
